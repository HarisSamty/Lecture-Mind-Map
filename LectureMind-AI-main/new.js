document.addEventListener('DOMContentLoaded', function () {
  ...
  if (window.location.hostname === 'mind-map-LectureMind.pages.dev') {
    window.location.href =
      'https://mindmapLectureMind.com' +
...
function initializeRatingSystem() {
  ...
  document.querySelectorAll('.rating input').forEach((input) => {
    input.addEventListener('change', function () {
      ...
      const scriptURL = 'https://post.mindmapLectureMind.com';
      const formData = new FormData();
      formData.set('rating', `${rating}/5`);
      formData.set('source', 'Mind Map LectureMind - Rate us');
...
function generateMindmapFromYoutube(youtubeUrl) {
  ...
  fetch('https://generate.mindmapLectureMind.com', {
    method: 'POST',
    headers: {
...
async function shareMindmap() {
  ...
  try {
    ...
    const response = await fetch('https://share.mindmapLectureMind.com/', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
async function generateMindmap(mindmapTopic, isRegenerate = false) {
  ...
  try {
    if (!isRegenerate) {
      ...
      const recentMindmapsElem = document.getElementById('recent-mindmaps');
      if (recentMindmapsElem) recentMindmapsElem.style.display = 'none';

      const aiDisclaimerElem = document.getElementById('ai-content-disclaimer');
      if (aiDisclaimerElem) aiDisclaimerElem.style.display = 'block';
    ...
    if (isRegenerate) {
      const regenerateBtn = document.getElementById('regenerate-button');
      if (regenerateBtn) regenerateBtn.classList.add('rotating');
    }
...
function renderMindmap(markdown) {
  ...
  try {
    ...
    mindmapContainer.style.height = '80vh';
...
function loadMindMapById(id) {
  ...
  if (mindMap.markdown) {
    try {
      ...
      document.title = `${currentMindmapTitle} | Mind Map LectureMind`;
